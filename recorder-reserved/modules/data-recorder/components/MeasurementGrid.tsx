...
import type { CalibrationRawDataSheet, CalibrationRawDataSheetInput, ForceUnit } from '../../../types';
import {
  SERIES,
  computeForce,
  fmtForce,
...
/** Series/column layout the grid renders — defaults to the fixed force/ISO-7500-1 SERIES. */
export type GridSeriesLayout = { key: string; label: string; group: 'inc' | 'dec' }[];
...
function GridHead({ unit, editable, series = SERIES }: { unit: ForceUnit; editable: boolean; series?: GridSeriesLayout }) {
  return (
    <thead>
      <tr>
...
interface EditableGridProps {
  ...
  /** Series/column layout to render — defaults to the fixed force/ISO-7500-1 SERIES. */
  series?: GridSeriesLayout;
}

export const MeasurementGrid: React.FC<EditableGridProps> = ({
  rows, options, readingUnit, decimalPlaces, onChange, series = SERIES,
}) => {
  ...
  const setCell = (index: number, seriesKey: string, field: 'uuc' | 'sig', value: string) => {
    const row = rows[index];
    setRow(index, {
      ...row,
      cells: { ...row.cells, [seriesKey]: { ...row.cells[seriesKey], [field]: value } },
    });
  };
  ...
  return (
    <div className="overflow-x-auto">
      <table className="border-collapse text-sm" style={{ minWidth: '1100px' }}>
        <GridHead unit={readingUnit} editable series={series} />
        <tbody>
          {rows.map((row, i) => {
...
export const ReadOnlyMeasurementGrid: React.FC<ReadOnlyGridProps> = ({ sheet, series = SERIES }) => {
  const unit = sheet.uuc.readingUnit;
  const dp = sheet.decimalPlaces;
  return (
    <div className="overflow-x-auto">
      <table className="border-collapse text-sm" style={{ minWidth: '1100px' }}>
        <GridHead unit={unit} editable={false} series={series} />
        <tbody>
