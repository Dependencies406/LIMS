...
export const ServiceRequestModal: React.FC<ServiceRequestModalProps> = ({
  ...
  const checkEquipmentBySerialNumber = async (index: number, serialNumber: string) => {
    try {
      const equipment = await (equipmentService as any).getEquipmentBySerialNumber(serialNumber);
      if (equipment) {
        setForm((prev) => {
          const newEquipment = [...prev.equipment];
          const base = {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!existingCustomer) {
        ...
        try {
          ...
          await customerService.createCustomer({
            ...
            contact: form.customerCompanyName,
      ...
      for (const eq of equipmentForWrite) {
        if (eq.serialNumber) {
          try {
            await (equipmentService as any).createOrUpdateEquipment({
