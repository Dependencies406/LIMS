...
export const RoleManagementModal: React.FC<RoleManagementModalProps> = ({
  ...
  const { hasPermission: canEditRoles } = usePermission('roles.edit');
  const { hasPermission: canDeleteRoles } = usePermission('roles.delete');
  const hasRoleAccess = isAdmin || canViewRoles;
