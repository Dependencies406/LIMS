...
export const EquipmentSpreadsheetModal: React.FC<EquipmentSpreadsheetModalProps> = ({
  ...
  const handleGenerateCertificateNumber = useCallback(() => {
    console.log('[EquipmentSpreadsheetModal] handleGenerateCertificateNumber called', {
      certificateConfigsCount: certificateConfigs.length,
      equipmentCertificateNumber: equipment.certificateNumber
    });
    ...
    if (equipment.certificateNumber !== null && equipment.certificateNumber !== undefined) {
      console.log('[EquipmentSpreadsheetModal] Certificate number already exists, button should be disabled');
      showError('Certificate number already generated for this equipment');
      return;
    }
    
    if (certificateConfigs.length === 0) {
      console.log('[EquipmentSpreadsheetModal] No certificate configs available');
      // Show modal error dialog instead of toast for better visibility
      setShowCertificateNumberError(true);
      return;
    }
    
    console.log('[EquipmentSpreadsheetModal] Opening confirmation dialog');
  ...
  const handleConfirmCertificateNumberGeneration = useCallback(async () => {
    console.log('[EquipmentSpreadsheetModal] handleConfirmCertificateNumberGeneration called', {
      equipmentIndex,
      equipmentCertificateNumber: equipment.certificateNumber,
      availableConfigs: certificateConfigs.length
    });
    
    // Single source of truth: Check if equipment already has a certificate number
    if (equipment.certificateNumber !== null && equipment.certificateNumber !== undefined) {
      console.log('[EquipmentSpreadsheetModal] Certificate number already exists on equipment');
      showError('Certificate number already generated for this equipment');
    ...
    try {
      ...
      if (onCertificateNumberGenerated) {
        console.log('[EquipmentSpreadsheetModal] Calling onCertificateNumberGenerated callback');
        onCertificateNumberGenerated(equipmentIndex, certificateNumber);
      }

      console.log('[EquipmentSpreadsheetModal] Equipment updated successfully');
      setShowCertificateNumberConfirm(false);
            ...
            <div className="p-5">
              ...
              {loadingTrebTemplates ? (
                <div className="flex items-center justify-center py-10">
                  <LoadingSpinner size="sm" message="Loading templatesâ€¦" />
                </div>
                ...
                <div className="text-center py-10">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Create spreadsheet templates in Settings â†’ Spreadsheet Template Builder
                  </p>
                </div>
              ) : (
