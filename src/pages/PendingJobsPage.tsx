...
export const PendingJobsPage: React.FC = () => {
  ...
  const { hasPermission: canView, loading: permLoading } = usePermission('serviceRequests.view');
  const { hasPermission: canConvert } = usePermission('serviceRequests.convert');
  const { hasPermission: canCancel } = usePermission('serviceRequests.cancel');
  ...
  // Load pending service requests — only if user has serviceRequests.view permission
  useEffect(() => {
    if (!canView) {
      setLoading(false);
      return;
  ...
  const handleConvertToJob = async (request: ServiceRequest) => {
    ...
    if (!canConvert) {
      showError('You do not have permission to convert service requests to jobs.');
      return;
    }
    ...
    try {
      ...
      const jobData = {
        ...
        startDate: '',
        appointmentDate:
          (request.requestedCalibrationDate || request.requestedCalibrationDate || '').trim(),
        comments: commentParts.join('\n\n'),
      };

      // Create job
      const createdJobId = await jobService.createJob(jobData as any, currentUser.uid);
  ...
  const handleCancelRequest = async (request: ServiceRequest) => {
    if (!canCancel) {
      showError('You do not have permission to cancel service requests.');
      return;
    }
  ...
  // Show access-denied screen if user lacks the view permission (and permissions have loaded)
    ...
    return (
      <div className="p-6">
        <div className="text-center py-16">
          ...
          <p className="text-sm text-gray-500">You don't have permission to view service requests.</p>
        </div>
      </div>
    );
  }
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    {(request.requestedCalibrationDate || request.requestedCalibrationDate) && (
                      <div>
