...
/**
 * Generate the seed FormulaSet for a given sheetType. Currently only
 * 'force-iso7500-1' is implemented; other sheetTypes throw, since design
 * §8b's multi-type registry (which sheetType maps to which step set) is
 * future work, not this session's scope.
 */
export function generateSeedFormulaSet(sheetType: string): FormulaSet {
  if (sheetType !== FORCE_ISO7500_1_SHEET_TYPE) {
    throw new Error(`No seed FormulaSet defined for sheetType '${sheetType}'`);
  }
  return {
    sheetType,
    steps: [...RELATIVE_ERROR_STEPS, ...UNCERTAINTY_BUDGET_STEPS],
  };
}
